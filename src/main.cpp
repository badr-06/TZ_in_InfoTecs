...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    if (processor.check(input, idx) == 1) {  // Если первый символ - цифра
      ...
      if (levelNum >= 0 && levelNum <= 2) {  // Проверяем допустимость уровня
        ...
        if (processor.check(input, idx) == 2) {
          // Если есть текст - извлекаем его и добавляем в лог с указанным
          // уровнем
        ...
        // Если уровень недопустим - логируем всё сообщение с уровнем по
        // умолчанию
    ...
    // Если ввод не начинается с цифры, но содержит текст
      ...
      processor.add(level,
                    message);  // Добавляем сообщение с уровнем по умолчанию
    }
  }
